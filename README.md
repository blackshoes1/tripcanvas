...
tailnet 밖에서 이 상태를 확인할 수 있는 곳은 Vercel뿐이라 `api/health-watch.js`가 외부에서 NAS를 확인한다. 저장 경로가 죽으면 503, 실시간만 죽었을 때는 폴백이 있어서 200 DEGRADED를 반환한다.
...
`/api/health`는 서비스가 살아 있는지만 확인한다.
