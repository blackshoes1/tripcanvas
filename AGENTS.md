...
- 화면 문구에 내부 용어를 쓰지 않는다: NextAction→**다음** · Suggestion→**From J** · Replan→**일정 조정 제안**(웹 `SG_KICKER.REPLAN` · iOS `ReplanCard` — 2026-09-21에 확인한 실제 라벨이다. 전에는 여기 '다시 맞추기'라고 적혀 있었지만 그 문구를 쓰는 화면이 없었다) · Collaboration→**같이 짜기** · Candidate→**가고 싶은 곳** · Conflict→**의견이 갈려요** · Travel Mode→**여행 중**.
  ⚠️ TripPulse는 이 목록에 없다 — 화면에 **머리글이 없기** 때문이다(`TripPulseBar`는 서버가 준 한 마디 `pulse.text`만 그린다). 이름이 없으면 내부 용어가 샐 일도 없다. 2026-09-21 전에는 여기 'TripPulse→오늘의 흐름'이라고 적혀 있었는데 그 문구를 쓰는 화면이 없었다.
...
- `next/src/features/trip-state/` — 웹·iOS 공통 API 계층. 아래 `domain/`에 산다: `domain/contract.ts`(단일 출처 계약) · `todayView.ts`(엔진 결과를 계약 모양으로) · `dayPlanView.ts`(**일정 화면이 쓰는 하루치** — `dayView.ts`의 계산을 계약 모양으로. ⚠️ **라벨이 아니라 값을 싣는다**: 웹 `DayView`는 `"📏 하루 동선 약 12.4km"` 같은 완성된 문장을 들고 있는데 그걸 보내면 앱이 표기를 정할 수 없다. ⚠️ 타임라인의 '분'은 소수라 **정수로 반올림해서** 보낸다 — 안 그러면 Swift가 `Int` 디코딩에서 죽고 그 사고는 앱 빌드까지 아무도 모른다) · `mutations.ts`(문서 변경 순수 함수) · `handlers.ts`(주입 가능한 라우트 핸들러) · `supabaseGateway.ts`(RLS 아래 읽기·쓰기)
...
- **상태 문구는 '결제 완료 / 결제 예정 / 고르지 않음'이다**(웹 `PAY_STATE_LABEL` = iOS `CostPayState.label`, **글자까지 같다**). '예약만 함'·'결제함'은 모호해서 버렸다.
  ⚠️ **이름과 '말할지 말지'는 다른 질문이다**(2026-09-20). 세 상태 모두 이름이 있어야 **고르는 칸**에 쓸 수 있고, 고르지 않은 것을 **줄·요약에 적지 않는 것**은 쓰는 쪽이 판단한다(웹 `payStateNote` · iOS의 `!= .none` 가드와 `paySplit`). 전에는 웹이 NONE의 이름을 빈 문자열로 둬서 숨기기를 대신했고 iOS만 '미구분'이라, 같은 값이 요약에서는 '미구분'·고르는 칸에서는 '고르지 않음'이었다. '미구분'은 이제 세는 쪽의 개념 이름(`NONE`)일 뿐 화면에 나오지 않는다. 두 장부의 이름은 **예약 결제 금액**(prep) / **현지 결제 금액**(onSite)이고 웹 필터바 전체 비용 내역도 이 둘로 묶는다. 환율 표기는 **원 단위 반올림**이고 엔은 100엔 기준이다("100 JPY ≈ 931원", iOS `TripCostsView.fxLine`).
...
**웹 색도 토큰이 말한다 — 다만 값이 같다고 리터럴을 토큰으로 바꾸면 안 된다.** 토큰은 테마에 따라 값이 바뀐다: `.btn.primary{color:#fff}`를 `var(--surface)`로 바꾸면 라이트에서는 같지만 다크에서 `#221e19`가 되어 **초록 배경에 검은 글자**가 된다. `#fff` 22곳이 전부 그런 '테마와 무관하게 흰색' 자리다. 토큰으로 묶을 것은 **라이트·다크 쌍으로 박혀 있던 것**이고 (상태 배지 `--badge-{good,warn,danger}-{bg,fg}`, 2026-09-21), 바꾼 뒤에는 `e2e/theme-tokens.spec.js`가 **계산된 색을 양쪽 테마에서 읽어** 값이 그대로인지 본다(jsdom은 CSS 변수를 끝까지 풀지 못해 통합 테스트로는 못 본다).
...
- **색은 뜻이다** — `Ink.accent`(누를 것·고른 것) · `Ink.warning`(시간 경고·미정·확인할 것) · `Ink.danger`(오류·삭제·초과·취소) · `Ink.positive`(완료) · `Ink.info`(상대가 정한 것). 화면이 `.orange`·`.red`·`.green`·`.blue`를 직접 부르지 않는다. 2026-09-21에 남아 있던 46건을 옮겨 **우회가 0**이 됐다. 예외는 하나뿐이다: 구글 지도 마커는 SDK가 `UIColor`를 요구하고 타일 위에서 읽혀야 해서 시스템 색을 쓴다(`GoogleMapContainer`) — 화면과 갈린 것이 아니라 **다른 매체**다. 늦게 끝나는 것은 경고가 아니다 — 자정을 넘길 때(`overloaded`)만 주의색.
...
- **날짜는 숫자로 쳐도 되고 달력을 눌러도 된다**(`DateEntryField` — `ISODateText.parseLoose`가 `20261025`·`2026.10.25`를 받고 8자리가 아니면 연도를 추측하지 않는다). 여행 기간은 **시작일–종료일**로 정하고 일수는 파생이다(iOS 여행·하루 설정·새 여행, 웹 여행 설정의 `#tripEnd` — 일수 칸과 서로 맞춰진다). 시작일이 없는 여행만 일수로 정한다.
  **기간 상한은 90일 하나다**(2026-09-20). 정하는 곳은 `lib.js`의 `TC_LIMITS.days`고 — 서버가 생성·수정 양쪽에서 `validateTripPayload`를 지나므로 그 숫자가 곧 저장 가능한 최대치다 — 웹(`#tripDays`의 `max`·`tripDaysInput`)과 iOS(`TripLimits.maxDays`, `NewTripDraft`·`PlanSettingsView`가 함께 본다)는 그 값을 미리 알아 **못 저장할 기간을 입력에서 막는다**. `next`의 `tripLimitsParity.test.ts`가 lib과 Swift를, 통합 테스트가 lib과 마크업을 대조한다.
...
- 자연어("오늘 좀 피곤해서 많이 걷기 싫어")는 `parseIntent`로 **옵션(energyLevel·maxTravelMin·walkAverse)만** 바꾼다. 충돌·운영시간·이동시간 판단은 그대로 deterministic 로직이 한다. 못 알아들으면 알아들은 척하지 말고 그렇게 말한다.
  **해석은 한 곳이고 앱은 클라이언트다**(2026-09-21). 웹은 `resolveIntent`를 직접 부르고, 앱은 문장을 `GET /api/v1/trips/:id/today?intent=…&energy=…`로 보내 서버가 같은 함수로 해석한 결과(`TodayResponse.intent` = `IntentEcho`)를 그린다 — 규칙을 Swift로 옮기면 같은 문장에 두 답이 나온다.
  ⚠️ **합치는 규칙은 `resolveIntent` 하나다**: 컨디션은 **문장이 말했을 때만** 덮어쓰고(안 말했으면 버튼 값이 남는다), 조건은 **문장이 통째로** 정한다(앞 문장의 걷기 제한이 "밥 먹자"에 따라붙지 않는다). 계약 밖의 컨디션 문자열은 보통으로 떨어진다.
  ⚠️ **저장하지 않는다.** 문장은 요청마다 오고 서버는 이번 계산에만 쓴다(위치와 같은 규칙). 기기 쪽도 웹·앱 모두 로컬이다.
  ⚠️ **문장을 보낸 응답은 디스크에 담지 않고, 담긴 것을 먼저 그리지도 않는다**(iOS) — 그 캐시는 문장 없이 받은 것이라 "배고파" 한 번이 다음 날 아침에 되살아나거나 에코 없는 옛 제안이 잠깐 보인다.
  ⚠️ **`travel-state`에는 문장을 싣지 않는다** — `stateVersion`에 컨디션이 들어 있어 타이핑마다 잠금화면·위젯이 다시 그려진다. 에코(`IntentEcho`)는 Today 응답에만 있고 압축본에는 없다(사람이 쓴 문장이 잠긴 화면에 떠 있으면 안 된다).
  ⚠️ `IntentEcho`는 **아는 키만 눕힌다**(`maxTravelMinutes`·`walkAverse`·`mealFocus`·`wantRest`) — 엔진이 내부 `prefs` 키를 늘려도 계약이 흔들리지 않고, 앱은 임의 딕셔너리를 디코딩할 수 없다. 좁히지 않은 상한은 **null**이다(0과 섞지 않는다).
...
**앱의 실시간은 여행이 붙고, 화면은 듣기만 한다.**(2026-09-20)

- 소켓은 **여행 하나에 하나**다. `TripHomeView`가 여행을 보는 동안 붙고(`key:"trip"`), '가고 싶은 곳' 보드가 제 구독을 더한다(`key:"candidates"`). ⚠️ **키가 필요한 이유**: 전에는 `RealtimeClient`가 핸들러를 하나만 들어, 이미 붙은 여행에 두 번째 화면이 붙으면 그 화면의 핸들러가 **조용히 버려졌다**. 이제 같은 키면 갈아 끼우고, `disconnect(key:)`는 **남은 구독자가 있으면 소켓을 닫지 않는다**(시트를 닫았다고 일정이 실시간을 잃으면 안 된다).
- 무엇을 다시 읽을지는 `TripLiveRefresh.decide`(순수)가 정한다 — `liveEffects`(`collab.js`가 소스)를 그대로 쓰고 **화면이 이미 내용을 들고 있을 때만** 켠다. 닫힌 시트는 열 때 `loadIfStale`이 챙기므로, 안 보는 화면 때문에 소켓마다 네트워크를 쓰지 않는다.
- 갈래: `pull`→`지금`·`일정` · `members`→함께하기 멤버 · `activity`→함께하기 활동 기록 · **후보 목록은 보드가 제 구독으로 받는다**(여기서도 읽으면 보드가 열렸을 때 두 번 받는다).
- ⚠️ 함께하기는 `load()`(4건)가 아니라 `applyLive(members:activity:)`로 **바뀐 것만** 읽는다 — #34가 없앤 "이름 하나 바꿔도 4건"이 소켓마다 되살아나면 안 된다.
- ⚠️ `pull`은 `doc && !mine`이라 **내 편집의 에코로는 일정을 다시 읽지 않는다**(내 기기 화면은 이미 그렇다). 반면 후보 목록은 내 것이어도 읽는다 — 날짜 표시는 서버가 문서를 보고 바꾼다.
- 백그라운드에서는 전부 끊고(§34) 복귀할 때 두 화면이 각자 다시 붙는다. 못 붙어도 앱은 그대로다 — 당겨서 새로고침이 폴백이다.
...
- `spot.who`(참여자 user_id 배열) — **비었으면 모든 여행자다**(§26). 기본값이라 저장하지 않는다.
  **고치는 곳은 장소 편집기 하나고 웹·iOS가 같다**(2026-09-21): 웹 `#spotWhoSection`의 칩 · iOS `SpotEditorView`의 '누가 가나요'. 고를 사람이 둘 이상이고 `canAssignWho`(=편집 권한)일 때만 뜬다 — 보기 권한은 의견만 낸다(§12).
  ⚠️ 켜고 끄는 규칙은 `collab.js`의 **`pickWho` 하나**다(iOS는 복사본, `who-text.json` 픽스처가 대조). **전원을 고르면 비운다** — '모두'와 뜻이 같은데 다른 모양으로 저장하면 `whoKey`가 `'u1,u2,u3'`와 `'*'`로 갈려 **갈라지지 않은 하루가 분리된 것처럼 보인다.** 담는 순서는 멤버 순서다(`whoKey`가 정렬하므로 갈라짐에는 영향이 없고, 같은 선택이 늘 같은 배열이어야 웹과 앱이 같은 문서를 만든다).
  ⚠️ **참여자를 고치는 것은 묶음을 만들거나 지우는 것이 아니다.** `who`는 그 장소가 *어느 가지*에 속하는지를 정하고, 묶음 키(`split`)는 그대로 물려받는다 — 아래 "장소 모달은 분리 묶음을 만들지도 지우지도 않는다"는 그 `split`에 대한 규칙이다. 웹은 `spotSave`가 `split`·`reunion`을 명시적으로 물려주고, iOS는 `TripSpot`이 원문 트리를 들고 아는 필드만 덮어 써서 저절로 남는다.
  ⚠️ **칩이 없는 id(나간 사람)를 저장에서 떨어뜨리지 않는다**(2026-09-21). 웹은 저장할 때 칩에서 다시 읽어 그런 id를 조용히 버렸다 — 이름 한 줄만 고쳐도 참여자가 '모두'로 바뀌었다. 지금은 고른 값을 `_pickedWho`에 들고 있고, 사람이 칩을 눌렀을 때만 `pickWho`가 모르는 id를 정리한다.
