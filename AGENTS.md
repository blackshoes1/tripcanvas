...
localStorage: `tripcanvas_v1`(여행) · `tripcanvas_legs_v4`(구간 캐시, 수단별 키) · `tripcanvas_sync_v2`(revision/재시도 상태, `tripcanvas_synced`에서 호환 이관) · `tripcanvas_prices_v1`(예약 가격 관측 기록) · `tripcanvas_cfg` · `tripcanvas_fx`
