...
**렌터카 픽업·반납은 일정에 '표시만' 한다.** `carEventsOn(bookings, iso)` (lib) — 그 날짜의 픽업(`start`)·반납(`end`)을 예약에서 파생한다. 픽업·반납 장소는 자유 텍스트라 **좌표가 없다** → 동선·ETA·앵커·지도에는 넣지 않는다. 사이드바에서는 `.spot.carbk` 행으로, 픽업은 장소 목록 앞·반납은 뒤(숙소 복귀 앞)에 놓는다. 시각은 ETA 칸이 아니라 메타 줄에 — ETA 칸은 '그날 계산된 도착 예상 순서'를 뜻하는데 이 항목은 그 순서에 속하지 않는다. 드래그 인덱스가 어긋나므로 `.spotList` 안에는 넣지 않는다.
