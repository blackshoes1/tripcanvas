...
```
같은 이메일로 **가입** → 확인 메일 → 링크로 이메일 확인 → resolveDomainUser가 기존 users 행에 연결 → 여행 그대로
```

⚠️ **재설정이 아니라 가입이다.** 2026-09-04 전환에서 실제로 여기서 막혔다: 예전 사용자는 새 Auth의 계정 행(`auth_user`)이
없으므로 재설정 요청이 `User not found`로 끝난다. 서버는 계정 유무를 알려주지 않으므로(§19의 다른 규칙) 메일도 오지 않고,
사용자는 아무 안내 없이 갇힌다. 로그인 실패 안내는 그래서 **가입**으로 보낸다.
