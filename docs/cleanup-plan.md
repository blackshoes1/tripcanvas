...
| **M8** (부분 완료) | ~~`spot.bookUrl`(디코딩만 하고 화면에 없다) · `day.flight`~~ **완료(2026-09-20)** — `지금` 카드의 `예약 열기`(+ `SafeURL.web`)와 일자 목록의 항공편 한 줄. 남은 것: **자연어 입력**(`parseIntent` — `adaptive.js`에만 있고 서버 라우트가 없다, 큼) · **`spot.who` 편집**(표시는 이미 된다. ⚠️ 분리 안의 장소에서 고치면 `whoKey`가 가지를 다시 갈라 "장소 모달은 분리 묶음을 만들지도 지우지도 않는다"와 부딪친다 — 분리 밖에서만 여는 것이 안전하다) | 예 — 남은 둘 |
