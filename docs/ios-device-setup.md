...
Mac 앞에 앉아서 이 문서만 따라가면 폰에 앱이 깔린다. 개발 경험이 없어도 되게 썼다.

목차: [준비물](#준비물) · [5분 경로(무료 계정)](#5분-경로-무료-apple-id) ·
[전체 기능 경로(유료)](#전체-기능-경로-유료-developer-program) · [기기 점검 체크리스트](#기기-점검-체크리스트) ·
[TestFlight](#testflight) · [오류가 났을 때](#오류가-났을-때)
...
| 무엇 | 비고 |
|---|---|
| **Mac** | Xcode가 macOS에서만 돈다. Windows·Linux로는 못 만든다 |
| **Xcode 15 이상** | App Store에서 설치. 10GB 넘으니 미리 받아둘 것 |
| **iPhone (iOS 17 이상)** | 앱이 `@Observable`·Live Activity를 써서 17이 최소다 |
| **케이블** | 첫 설치는 유선이 확실하다 |
| **Apple ID** | 무료로 시작할 수 있다. 아래 두 경로 참고 |

**두 갈래를 먼저 고른다.**

|  | 무료 Apple ID | 유료 Developer Program (연 $99) |
|---|---|---|
...
App Group과 푸시 권한이 유료 프로그램에서만 발급되기 때문이다. 앱을 처음 보는 게 목적이면
무료로 시작해도 충분하다 — **코드는 손대지 않아도 된다.** App Group 접근은 권한이 없으면
`nil`로 떨어지고 Live Activity는 조용히 건너뛰도록 짜여 있다.
...
Xcode 프로젝트 파일은 저장소에 넣지 않는다(손으로 고치면 잘 깨진다). 대신 `project.yml`
명세에서 만들어 쓴다.
...
Xcode 왼쪽에서 **TripCanvas** 프로젝트 클릭 → 가운데 **TARGETS → TripCanvas** →
**Signing & Capabilities** 탭:

- **Automatically manage signing** 체크
- **Team**: Apple ID 선택. 목록에 없으면 `Add an Account...`로 로그인하면 `(Personal Team)`이 생긴다
...
## 전체 기능 경로 (유료 Developer Program)

위젯·Live Activity·공유 확장·Watch까지 보려면 유료 멤버십에 가입한 뒤:

```bash
cd ios
xcodegen generate            # project.yml (기본 스펙)
open TripCanvas.xcodeproj
```

**모든 타깃에 Team을 지정해야 한다.** 하나라도 빠지면 빌드가 서명에서 멈춘다:

```
TripCanvas          TripCanvasWidgets   TripCanvasShare
TripCanvasWatch     TripCanvasTests
```

각 타깃의 Signing & Capabilities에서 같은 Team을 고른다.
...
설치가 됐으면 실제로 도는지 확인한다. 아래는 폰을 손에 들고 순서대로 짚어가는 목록이다.
...
매번 Xcode로 설치하지 않으려면 TestFlight를 쓴다. **유료 프로그램이 필요하다.**

### 올리기 전에

- [ ] `ios/project.yml` 의 `CURRENT_PROJECT_VERSION` 을 **올린다** — 같은 빌드 번호는 두 번 못 올린다
- [ ] Release 빌드가 되는지: Xcode → Product → Scheme → Edit Scheme → Run → Build Configuration을 **Release**로 바꿔 한 번 실행해 본다
- [ ] `aps-environment` 확인 — `project.yml`에 `development`로 돼 있다. TestFlight 빌드는 **production APNs**를 쓰므로, 원격 푸시를 테스트하려면 배포용 프로비저닝에 맞춰야 한다 (자동 서명이면 Xcode가 배포 프로파일로 다시 서명한다 — 푸시가 안 오면 여기부터 의심할 것)

### 순서

1. Xcode 상단 실행 대상을 **Any iOS Device (arm64)** 로 바꾼다
2. **Product → Archive**
3. Organizer 창이 열리면 **Distribute App → TestFlight & App Store Connect**
4. [App Store Connect](https://appstoreconnect.apple.com) → TestFlight → 내부 테스터에 자기 계정 추가
5. 폰에 TestFlight 앱을 깔고 초대 수락

처음 한 번은 App Store Connect에 앱 레코드를 만들어야 한다 (My Apps → + → New App,
Bundle ID는 `ai.tripcanvas.ios`).
...
2. 폰에서 **이 컴퓨터를 신뢰** 했는지
3. **개발자 모드**가 켜져 있는지 (설정 → 개인정보 보호 및 보안 → 개발자 모드) → 켠 뒤 **재시동 필요**
...
무료 계정의 정상 동작이다. Mac에 연결해 `⌘R` 로 다시 설치한다.
매번 하기 싫으면 유료 프로그램 + TestFlight로 간다.

### 위젯이 목록에 안 나온다

순서대로 의심한다:

1. **무료 계정인가?** → App Group이 없어서 정상적으로 안 뜬다. 유료가 필요하다
2. 앱을 **한 번은 실행**했는가? 위젯은 앱이 App Group에 써 둔 값을 읽는다 — 쓴 게 없으면 안 뜬다
3. App Group 문자열이 세 곳에서 같은가? (위 [전체 기능 경로](#전체-기능-경로-유료-developer-program) 표)
...
- TestFlight 빌드인데 안 오면 `aps-environment` 를 의심한다 (위 [TestFlight](#testflight) 참고)
