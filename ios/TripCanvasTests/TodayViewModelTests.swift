...
final class TodayViewModelTests: XCTestCase {
    ...
    func testEnteringTheTabAgainDoesNotAskTheServerWhileFresh() async throws {
        ...
        XCTAssertTrue(await model.loadIfStale(), "처음에는 받는다")
        XCTAssertFalse(await model.loadIfStale(), "방금 받았으면 묻지 않는다")
        XCTAssertFalse(await model.loadIfStale(), "몇 번을 들어와도 같다")
        XCTAssertEqual(stub.todayCallCount, 1)
        XCTAssertTrue(await model.loadIfStale(now: Date().addingTimeInterval(120)), "오래됐으면 뒤에서 새로 받는다")
    ...
    func testOfflineContentIsNeverConsideredFresh() async throws {
        ...
        XCTAssertTrue(await model.loadIfStale(), "캐시로 그린 것은 다시 묻는다")
        XCTAssertEqual(stub.todayCallCount, 2)
    }
