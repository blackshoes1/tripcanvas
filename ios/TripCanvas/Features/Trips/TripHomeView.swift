...
///
/// ⚠️ `일정`과 `지도`는 **같은 화면의 두 모드다** — `TripPlanView` 하나가 문서·고른 날짜·편집기를
/// 들고 있으므로 탭마다 새로 만들면 두 번 불러오고 고른 날이 갈린다. 그래서 `TabView`가 아니라
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            case .today:
                TodayView(trip: trip)
            case .plan, .map:
                // 한 갈래다 — 뷰 정체성이 유지돼 탭을 오가도 문서를 다시 불러오지 않는다.
                TripPlanView(trip: trip, showsMap: Binding(
