...
- ⚠️ 2026-09-21까지 여기 "아직 없는 것"으로 **실시간 · 지도에서 후보 담기 · 그룹 제안 카드 · 분리 일정** 넷이
  적혀 있었는데, **넷 다 이미 들어와 있었다** — 이 문서 아래쪽(`함께하기 — 지도에서 담기와 그룹 제안`,
  `실시간`)이 그 기능들을 설명하고 있어 같은 파일 안에서 앞뒤가 달랐다. 분리 일정도 갈린 후보의
  '자유시간으로 분리'로 들어갔다. **없다고 적은 목록은 기능이 들어올 때 같이 지워야 한다.**
