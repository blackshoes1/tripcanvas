...
## 함께하기 — 후보 담기와 의견은 폰이 제자리다

지도 위의 본격적인 계획은 웹이 하지만, **가고 싶은 곳을 담고 하트를 누르고 한마디를 남기는 일**은
침대에서 10초 안에 하는 행동이라 앱에 있다. Today 화면 오른쪽 위의 **함께하기** 메뉴로 들어간다.

- `Services/CollabService.swift` — 경로와 본문만 만든다
- `Features/Collab/` — 보드 · 코멘트 · 취향 화면

묶음·배지 문장·갈렸을 때의 선택지·그룹 제안은 **서버가 이미 만들어 보낸다**(`candidatesView.ts`).
그래서 반응 하나를 눌러도 응답이 보드 전체다 — 그 파급을 앱이 흉내내면 웹과 답이 갈린다.
자세한 것은 [docs/collaboration.md](../docs/collaboration.md).
