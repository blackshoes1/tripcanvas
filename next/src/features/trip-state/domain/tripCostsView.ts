...
export function buildTripCosts(trip: TripDoc, legCache: LegCache, revision: number): TripCostsResponse {
  const document = trip as unknown as Trip;
  const days = document.days.map((day, index) => {
    const cost = dayCostPartsOf(document, legCache, index, FX_FALLBACK);
  ...
  return { schemaVersion: CONTRACT_SCHEMA_VERSION, revision, days,
    ...lib.tripCostSummary(trip, days, FX_FALLBACK),
    fxRates: { ...FX_FALLBACK }, fxSource: 'FALLBACK', fxAsOf: null };
}
